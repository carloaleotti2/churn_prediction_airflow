...
INPUT_PATH = "C:\\airflow-project\\dataset_cleaned.csv"  # dataset già pulito da EDA (num_cols numerici, cat_cols object)
...
# --- NUOVO: tuning soglia decisionale ---
# Perché: la soglia 0.5 sopra è arbitraria e non riflette il business case
# (FN più costoso di FP). Cerco la soglia che massimizza il recall mantenendo
# una precision minima accettabile (qui 0.5, da tarare col business).
MIN_PRECISION = 0.5
...
if valid.any():
    ...
    pred_tuned = (xgb_proba >= best_threshold).astype(int)
    print(f"\n--- XGBoost (soglia ottimizzata = {best_threshold:.3f}, "
          f"precision >= {MIN_PRECISION}) ---")
    print(f"Recall:    {recall_score(y_test, pred_tuned):.4f}")
    print(f"Precision: {precision_score(y_test, pred_tuned):.4f}")
    print(f"F1:        {f1_score(y_test, pred_tuned):.4f}")
    print(confusion_matrix(y_test, pred_tuned))
else:
    best_threshold = 0.5
    print(f"\nNessuna soglia raggiunge precision >= {MIN_PRECISION}, mantengo 0.5")
